...
//function to make decimals into binary
...
void ButtonISR(void) {
    ...
    uint32_t gpio_buttons =
            ...
            ((~GPIOPinRead(GPIO_PORTH_BASE, 0xff) & (GPIO_PIN_1)) << 1) |       //button S1
            ((~GPIOPinRead(GPIO_PORTK_BASE, 0xff) & (GPIO_PIN_6)) >> 3) |       //button S2
    ...
    if (presses & 1) { // EK-TM4C1294XL button 1 pressed pause/start
        running = !running;
    }

    if (presses & 2) {
        gTime=0;    //when USR_SW2 is pressed then restart timer
    }
